...
def main():
    ...
    parser.add_argument(
        "--extract_only",
        action="store_true",
        help="Skip training and only export features from saved checkpoint.",
    )
    ...
    train_eval_ds = EvalCIFAR10(str(data_dir), train=True, download=True)
    test_ds = EvalCIFAR10(str(data_dir), train=False, download=True)
    ...
    ckpt_path = out_dir / f"dinov2_resnet18_seed{args.seed}.pth"
    if args.extract_only:
        if not ckpt_path.exists():
            raise FileNotFoundError(f"Checkpoint not found for --extract_only: {ckpt_path}")
        state = torch.load(ckpt_path, map_location="cpu")
        teacher.load_state_dict(state["teacher"])
        print(f"[Seed {args.seed}] Loaded checkpoint: {ckpt_path}")
    else:
        ...
        torch.save({"teacher": teacher.state_dict(), "seed": args.seed}, ckpt_path)
